...
func (rc *RoundCreate) AddChecklogIDs(ids ...int) *RoundCreate {
	rc.mutation.AddChecklogIDs(ids...)
	return rc
}

// AddChecklogs adds the "checklogs" edges to the CheckLog entity.
func (rc *RoundCreate) AddChecklogs(c ...*CheckLog) *RoundCreate {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return rc.AddChecklogIDs(ids...)
}
...
func (rc *RoundCreate) createSpec() (*Round, *sqlgraph.CreateSpec) {
	...
	if nodes := rc.mutation.ChecklogsIDs(); len(nodes) > 0 {
