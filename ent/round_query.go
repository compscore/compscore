...
func (rq *RoundQuery) QueryChecklogs() *CheckLogQuery {
	query := (&CheckLogClient{config: rq.config}).Query()
...
func (rq *RoundQuery) Clone() *RoundQuery {
	...
	return &RoundQuery{
		...
		withChecklogs: rq.withChecklogs.Clone(),
...
func (rq *RoundQuery) WithChecklogs(opts ...func(*CheckLogQuery)) *RoundQuery {
	query := (&CheckLogClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withChecklogs = query
	return rq
}
...
func (rq *RoundQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Round, error) {
	...
	if query := rq.withChecklogs; query != nil {
		if err := rq.loadChecklogs(ctx, query, nodes,
			func(n *Round) { n.Edges.Checklogs = []*CheckLog{} },
			func(n *Round, e *CheckLog) { n.Edges.Checklogs = append(n.Edges.Checklogs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoundQuery) loadChecklogs(ctx context.Context, query *CheckLogQuery, nodes []*Round, init func(*Round), assign func(*Round, *CheckLog)) error {
	...
	query.Where(predicate.CheckLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(round.ChecklogsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.check_log_round
		if fk == nil {
			return fmt.Errorf(`foreign-key "check_log_round" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "check_log_round" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
