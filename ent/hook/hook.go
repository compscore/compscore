...
// The CheckLogFunc type is an adapter to allow the use of ordinary
// function as CheckLog mutator.
type CheckLogFunc func(context.Context, *ent.CheckLogMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f CheckLogFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.CheckLogMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.CheckLogMutation", m)
}
