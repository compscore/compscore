...
func (cq *CheckQuery) QueryChecklogs() *CheckLogQuery {
	query := (&CheckLogClient{config: cq.config}).Query()
...
func (cq *CheckQuery) Clone() *CheckQuery {
	...
	return &CheckQuery{
		...
		withChecklogs: cq.withChecklogs.Clone(),
...
func (cq *CheckQuery) WithChecklogs(opts ...func(*CheckLogQuery)) *CheckQuery {
	query := (&CheckLogClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withChecklogs = query
	return cq
}
...
func (cq *CheckQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Check, error) {
	...
	if query := cq.withChecklogs; query != nil {
		if err := cq.loadChecklogs(ctx, query, nodes,
			func(n *Check) { n.Edges.Checklogs = []*CheckLog{} },
			func(n *Check, e *CheckLog) { n.Edges.Checklogs = append(n.Edges.Checklogs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CheckQuery) loadChecklogs(ctx context.Context, query *CheckLogQuery, nodes []*Check, init func(*Check), assign func(*Check, *CheckLog)) error {
	...
	query.Where(predicate.CheckLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(check.ChecklogsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.check_log_check
		if fk == nil {
			return fmt.Errorf(`foreign-key "check_log_check" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "check_log_check" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
