...
func (tc *TeamCreate) AddChecklogIDs(ids ...int) *TeamCreate {
	tc.mutation.AddChecklogIDs(ids...)
	return tc
}

// AddChecklogs adds the "checklogs" edges to the CheckLog entity.
func (tc *TeamCreate) AddChecklogs(c ...*CheckLog) *TeamCreate {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return tc.AddChecklogIDs(ids...)
}
...
func (tc *TeamCreate) createSpec() (*Team, *sqlgraph.CreateSpec) {
	...
	if nodes := tc.mutation.ChecklogsIDs(); len(nodes) > 0 {
