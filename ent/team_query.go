...
// QueryChecklogs chains the current query on the "checklogs" edge.
func (tq *TeamQuery) QueryChecklogs() *CheckLogQuery {
	query := (&CheckLogClient{config: tq.config}).Query()
...
func (tq *TeamQuery) Clone() *TeamQuery {
	...
	return &TeamQuery{
		...
		withChecklogs: tq.withChecklogs.Clone(),
...
// the "checklogs" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TeamQuery) WithChecklogs(opts ...func(*CheckLogQuery)) *TeamQuery {
	query := (&CheckLogClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withChecklogs = query
	return tq
}
...
func (tq *TeamQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Team, error) {
	...
	if query := tq.withChecklogs; query != nil {
		if err := tq.loadChecklogs(ctx, query, nodes,
			func(n *Team) { n.Edges.Checklogs = []*CheckLog{} },
			func(n *Team, e *CheckLog) { n.Edges.Checklogs = append(n.Edges.Checklogs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TeamQuery) loadChecklogs(ctx context.Context, query *CheckLogQuery, nodes []*Team, init func(*Team), assign func(*Team, *CheckLog)) error {
	...
	query.withFKs = true
	query.Where(predicate.CheckLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(team.ChecklogsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.check_log_team
		if fk == nil {
			return fmt.Errorf(`foreign-key "check_log_team" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "check_log_team" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
